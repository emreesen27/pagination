...
class DataSource {
    ...
    // handler, pagination result sınıfında beklenmediği için fonksiyonu düzenledim.
    suspend fun fetch(next: String?): Pair<FetchResponse?, FetchError?> {
        val processResult = processRequest(next)
        withContext(Dispatchers.Default) {
            delay((processResult.waitTime * 1000).toLong())
        }
        return Pair(processResult.fetchResponse, processResult.fetchError)
    }
